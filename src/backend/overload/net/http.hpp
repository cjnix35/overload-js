    ...
    namespace net {
        ...
        namespace http {
            ...
            using MultipartFormDataMap =
                std::multimap<std::string, MultipartFormData>;
            ...
            class DataSink {
                public:
                    ...
                    std::function<bool(const char *data, size_t data_len)>
                        write;
            ...
            using ContentProvider = std::function<bool(
                size_t offset, size_t length, DataSink &sink)>;

            using ContentProviderWithoutLength =
                std::function<bool(size_t offset, DataSink &sink)>;

            using ContentProviderResourceReleaser =
            ...
            class ContentReader {
                public:
                    using Reader =
                        std::function<bool(ContentReceiver receiver)>;
                    ...
                    bool operator()(MultipartContentHeader header,
                        ...
                        return multipart_reader_(std::move(header),
                                                 std::move(receiver));
                    }
            ...
            struct Request {
                    ...
                    std::string get_header_value(const std::string &key,
                                                 size_t id = 0) const;
                    template <typename T>
                    T get_header_value(const std::string &key,
                                       size_t id = 0) const;
            ...
            class Server {
                public:
                    ...
                    using HandlerWithResponse = std::function<HandlerResponse(
                ...
                private:
                    ...
                    using HandlersForContentReader = std::vector<
                        std::pair<std::regex, HandlerWithContentReader>>;

                    socket_t create_server_socket(
                        const std::string &host, int port, int socket_flags,
                        SocketOptions socket_options) const;
                    int bind_internal(const std::string &host, int port,
                                      int socket_flags);
                    ...
                    void apply_ranges(const Request &req, Response &res,
                                      std::string &content_type,
                                      std::string &boundary);
                    bool write_response(Stream &strm, bool close_connection,
                    ...
                    bool write_content_with_provider(
                        ...
                        const std::string &content_type);
                    bool read_content(Stream &strm, Request &req,
                                      Response &res);
            ...
            class Result {
                public:
                    ...
                    std::string get_request_header_value(const std::string &key,
                                                         size_t id = 0) const;
                    template <typename T>
                    T get_request_header_value(const std::string &key,
                                               size_t id = 0) const;
                    size_t get_request_header_value_count(
            ...
            class ClientImpl {
                public:
                    ...
                    Result Put(const std::string &path, const char *body,
                               size_t content_length,
                               const std::string &content_type);
                ...
                protected:
                    ...
                    // Also, shutdown_ssl and close_socket should also NOT be
                    // called concurrently with a DIFFERENT thread sending
                    // requests using that socket.
                    virtual void shutdown_ssl(Socket &socket,
                                              bool shutdown_gracefully);
                    void shutdown_socket(Socket &socket);
                    void close_socket(Socket &socket);

                    bool process_request(Stream &strm, Request &req,
                ...
                private:
                    ...
                    bool handle_request(Stream &strm, Request &req,
                                        Response &res, bool close_connection,
                                        Error &error);
            ...
            class SSLClient : public ClientImpl {
                ...
                private:
                    ...
                    void shutdown_ssl(Socket &socket,
                                      bool shutdown_gracefully) override;
                    void shutdown_ssl_impl(Socket &socket,
                    ...
                    bool connect_with_proxy(Socket &sock, Response &res,
                    ...
                    bool verify_host_with_subject_alt_name(
                        X509 *server_cert) const;
                    bool verify_host_with_common_name(X509 *server_cert) const;
                    bool check_host_name(const char *pattern,
            ...
            namespace detail {
                ...
                inline void duration_to_sec_and_usec(const T &duration,
                                                     U callback) {
                    ...
                    auto usec =
                        std::chrono::duration_cast<std::chrono::microseconds>(
                            ...
                            .count();
                    callback(static_cast<time_t>(sec),
                             static_cast<time_t>(usec));
                }
            ...
            template <typename T>
            inline T Request::get_header_value(const std::string &key,
                                               size_t id) const {
                return detail::get_header_value<T>(headers, key, id, 0);
            }

            template <typename T>
            inline T Response::get_header_value(const std::string &key,
                                                size_t id) const {
                return detail::get_header_value<T>(headers, key, id, 0);
            }

            template <typename... Args>
            inline ssize_t Stream::write_format(const char *fmt,
                                                const Args &...args) {
                ...
                if (n >= buf.size() - 1) {
                    ...
                    while (n >= glowable_buf.size() - 1) {
                        ...
                        n = static_cast<size_t>(
                            snprintf(&glowable_buf[0], glowable_buf.size() - 1,
                                     fmt, args...));
                    }
            ...
            inline std::ostream &operator<<(std::ostream &os,
                ...
                os << to_string(obj);
                os << " ("
                   << static_cast<std::underlying_type<Error>::type>(obj)
                   << ')';
                return os;
            }
            ...
            inline T Result::get_request_header_value(const std::string &key,
                                                      size_t id) const {
                return detail::get_header_value<T>(request_headers_, key, id,
                                                   0);
            }
            ...
            /*
             * Forward declarations and types that will be part of the .h file
             * if split into .h + .cc.
             */
            ...
            namespace detail {
                ...
                class compressor {
                    public:
                        ...
                        typedef std::function<bool(const char *data,
                                                   size_t data_len)>
                            Callback;
                ...
                // `fixed_buffer` to store data. The call can set memory on
