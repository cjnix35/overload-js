...
namespace over {
    ...
    namespace net {
        ...
        namespace http {
            ...
            /*
             * Implementation that will be part of the .cc file if split into .h
             * + .cc.
             */
            ...
            namespace detail {
                ...
                size_t to_utf8(int code, char *buff) {
                    if (code < 0x0080) {
                        ...
                        buff[0] = static_cast<char>(0xC0 |
                                                    ((code >> 6) & 0x1F));
                        buff[1] = static_cast<char>(0x80 | (code & 0x3F));
                        return 2;
                    } else if (code < 0xD800) {
                        buff[0] = static_cast<char>(0xE0 |
                                                    ((code >> 12) & 0xF));
                        buff[1] = static_cast<char>(0x80 |
                                                    ((code >> 6) & 0x3F));
                        ...
                        buff[0] = static_cast<char>(0xF0 |
                                                    ((code >> 18) & 0x7));
                        buff[1] = static_cast<char>(0x80 |
                                                    ((code >> 12) & 0x3F));
                        buff[2] = static_cast<char>(0x80 |
                                                    ((code >> 6) & 0x3F));
                ...
                // NOTE: This code came up with the following stackoverflow
                // post:
                        ...
                        switch (s[i]) {
                            ...
                            default:
                                ...
                                if (c >= 0x80) {
                                    result += '%';
                                    char hex[4];
                                    auto len = snprintf(hex, sizeof(hex) - 1,
                                                        "%02X", c);
                                    assert(len == 2);
                                    result.append(hex,
                ...
                std::pair<size_t, size_t> trim(const char *b, const char *e,
                ...
                void stream_line_reader::append(char c) {
                    if (fixed_buffer_used_size_ < fixed_buffer_size_ - 1) {
                        ...
                        if (glowable_buffer_.empty()) {
                            assert(fixed_buffer_[fixed_buffer_used_size_] ==
                                   '\0');
                ...
                ssize_t read_socket(socket_t sock, void *ptr, size_t size,
                                    int flags) {
                    return handle_EINTR([&]() {
                        return recv(sock,
#ifdef _WIN32
                ...
                ssize_t select_read(socket_t sock, time_t sec, time_t usec) {
                    ...
                    return handle_EINTR([&]() {
                        return select(static_cast<int>(sock + 1), &fds, nullptr,
                                      nullptr, &tv);
                    });
#endif
                }
                ...
                ssize_t select_write(socket_t sock, time_t sec, time_t usec) {
                    ...
                    return handle_EINTR([&]() {
                        return select(static_cast<int>(sock + 1), nullptr, &fds,
                                      nullptr, &tv);
                    });
#endif
                }
                ...
                Error wait_until_socket_is_ready(socket_t sock, time_t sec,
                    ...
                    auto ret = handle_EINTR([&]() {
                        return select(static_cast<int>(sock + 1), &fdsr, &fdsw,
                                      &fdse, &tv);
                    });
                ...
                bool is_socket_alive(socket_t sock) {
                    ...
                    char buf[1];
                    return detail::read_socket(sock, &buf[0], sizeof(buf),
                                               MSG_PEEK) > 0;
                }
                ...
                bool process_server_socket(
                    ...
                    return process_server_socket_core(
                        svr_sock, sock, keep_alive_max_count,
                ...
                socket_t create_socket(const std::string &host,
                                       ...
                                       int address_family, int socket_flags,
                                       bool tcp_nodelay,
                    ...
                    for (auto rp = result; rp; rp = rp->ai_next) {
                        // Create a socket
#ifdef _WIN32
                        auto sock = WSASocketW(rp->ai_family, rp->ai_socktype,
                                               rp->ai_protocol, nullptr, 0,
                                               WSA_FLAG_NO_HANDLE_INHERIT |
                                                   WSA_FLAG_OVERLAPPED);
                        /**
                         * Since the WSA_FLAG_NO_HANDLE_INHERIT is only
                         * supported on Windows 7 SP1 and above the socket
                         * creation fails on older Windows Systems.
                         *
                         * Let's try to create a socket the old way in this
                         * case.
                         *
                         * Reference:
...
#else
                        auto sock = socket(rp->ai_family, rp->ai_socktype,
                                           rp->ai_protocol);
#endif
...
#define USE_IF2IP
                ...
                void get_remote_ip_and_port(socket_t sock, std::string &ip,
                                            int &port) {
                ...
                namespace udl {
                    ...
                    constexpr unsigned int operator"" _t(const char *s,
                                                         size_t l) {
                ...
                bool nocompressor::compress(const char *data,
                                            size_t data_length, bool /*last*/,
                                            Callback callback) {
                ...
                gzip_compressor::gzip_compressor() {
                    ...
                    is_valid_ = deflateInit2(&strm_, Z_DEFAULT_COMPRESSION,
                                             Z_DEFLATED, 31, 8,
                ...
                bool gzip_compressor::compress(const char *data,
                                               size_t data_length, bool last,
                                               Callback callback) {
                    assert(is_valid_);
                    ...
                    do {
                        ...
                        auto flush = (last && data_length == 0) ? Z_FINISH :
                                                                  Z_NO_FLUSH;
                        int ret = Z_OK;
                ...
                gzip_decompressor::gzip_decompressor() {
                    ...
                    // possible value to ensure that any gzip stream can be
                    // decoded. The offset of 32 specifies that the stream type
                    // should be automatically detected either gzip or deflate.
                ...
                bool gzip_decompressor::is_valid() const {
                    return is_valid_;
                }

                bool gzip_decompressor::decompress(const char *data,
                                                   size_t data_length,
                                                   Callback callback) {
                    assert(is_valid_);
                ...
                brotli_compressor::brotli_compressor() {
                    state_ = BrotliEncoderCreateInstance(nullptr, nullptr,
                                                         nullptr);
                }
                ...
                bool brotli_compressor::compress(const char *data,
                                                 size_t data_length, bool last,
                                                 Callback callback) {
                    std::array<uint8_t, CPPHTTPLIB_COMPRESSION_BUFSIZ> buff{};

                    auto operation = last ? BROTLI_OPERATION_FINISH :
                ...
                bool brotli_decompressor::is_valid() const {
                    return decoder_s;
                }
                ...
                bool has_header(const Headers &headers,
                ...
                bool read_content_without_length(
                ...
                bool read_content_chunked(Stream &strm,
                    ...
                    while (true) {
                        ...
                        chunk_len = std::strtoul(line_reader.ptr(), &end_ptr,
                                                 16);
                ...
                bool prepare_content_receiver(
                    T &x, int &status, ContentReceiverWithProgress receiver,
...
#ifdef CPPHTTPLIB_BROTLI_SUPPORT
                            decompressor =
                                detail::make_unique<brotli_decompressor>();
#else
                            status = 415;
                            return false;
#endif
                ...
                template <typename T>
                bool read_content(Stream &strm, T &x, size_t payload_max_length,
                    ...
                    return prepare_content_receiver(
                        ...
                        [&](const ContentReceiverWithProgress &out) {
                            ...
                            if (is_chunked_transfer_encoding(x.headers)) {
                                ...
                                auto len = get_header_value<uint64_t>(
                ...
                bool write_content(Stream &strm,
                    ...
                    while (offset < end_offset && !is_shutting_down()) {
                        if (!content_provider(offset, end_offset - offset,
                                              data_sink)) {
                ...
                template <typename T>
                bool write_content_without_length(
                ...
                bool write_content_chunked(
                    ...
                    data_sink.write = [&](const char *d, size_t l) -> bool {
                        if (ok) {
                            ...
                            if (compressor.compress(
                                ...
                                if (!payload.empty()) {
                                    // Emit chunked response header and footer
                                    // for each chunk
                    ...
                    data_sink.done = [&](void) {
                        ...
                        if (!write_data(strm, done_marker.data(),
                                        done_marker.size())) {
                            ok = false;
                        }
                    };
                ...
                bool write_content_chunked(
                    ...
                    auto error = Error::Success;
                    return write_content_chunked(strm, content_provider,
                ...
                bool parse_range_header(const std::string &s,
                                        Ranges &ranges) try {
#endif
                    static auto re_first_range = std::regex(
                        R"(bytes=(\d*-\d*(?:,\s*\d*-\d*)*))");
                    std::smatch m;
                    if (std::regex_match(s, m, re_first_range)) {
                        ...
                        split(&s[pos], &s[pos + len], ',',
                              [&](const char *b, const char *e) {
                                  ...
                                  static auto re_another_range = std::regex(
                                      R"(\s*(\d*)-(\d*))");
                                  std::cmatch cm;
                                  if (std::regex_match(b, e, cm,
                                                       re_another_range)) {
                                      ssize_t first = -1;
                                      if (!cm.str(1).empty()) {
                ...
                class MultipartFormDataParser {
                    ...
                    private:
                        ...
                        bool
                            start_with_case_ignore(const std::string &a,
                        ...
                        // Buffer
                        bool start_with(const std::string &a, size_t spos,
                                        size_t epos,
                                        const std::string &b) const {
                ...
                std::string make_multipart_data_boundary() {
                    ...
                    // std::random_device might actually be deterministic on
                    // some platforms, but due to lack of support in the c++
                    // standard library, doing better requires either some ugly
                    // hacks or breaking portability.
                    std::random_device seed_gen;

                    // Request 128 bits of entropy for initialization
                    std::seed_seq seed_sequence{seed_gen(), seed_gen(),
                                                seed_gen(), seed_gen()};
                ...
                bool is_multipart_boundary_chars_valid(
                ...
                std::string serialize_multipart_formdata(
                    const MultipartFormDataItems &items,
                    const std::string &boundary, std::string &content_type) {
                    std::string body;
                ...
                std::pair<size_t, size_t> get_range_offset_and_length(
                    ...
                    if (r.first == -1) {
                        r.first = (std::max)(static_cast<ssize_t>(0),
                                             slen - r.second);
                        r.second = slen - 1;
                    }
                ...
                size_t get_multipart_ranges_data_length(
                    const Request &req, Response &res,
                    const std::string &boundary,
                    const std::string &content_type) {
                ...
                bool write_multipart_ranges_data(
                    ...
                    return process_multipart_ranges_data(
                        ...
                        [&](size_t offset, size_t length) {
                            return write_content(strm, res.content_provider_,
                                                 offset, length,
                                                 is_shutting_down);
                        });
                }
                ...
                std::pair<size_t, size_t> get_range_offset_and_length(
                    ...
                    if (r.second == -1) {
                        r.second = static_cast<ssize_t>(res.content_length_) -
                                   1;
                    }
...
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
                std::string message_digest(const std::string &s,
                                           const EVP_MD *algo) {
                    auto context =
                ...
                bool load_system_certs_on_windows(X509_STORE *store) {
                    auto hStore = CertOpenSystemStoreW((HCRYPTPROV_LEGACY)NULL,
                                                       L"ROOT");
                ...
                std::pair<std::string, std::string>
                    ...
                    std::string response;
                    {
                        ...
                        auto A1 = username + ":" + auth.at("realm") + ":" +
                                  password;
                    ...
                    auto opaque = (auth.find("opaque") != auth.end()) ?
                                      auth.at("opaque") :
                                      "";
                    ...
                    auto key = is_proxy ? "Proxy-Authorization" :
                                          "Authorization";
                    return std::make_pair(key, field);
                }
#endif

                bool parse_www_authenticate(
                    const Response &res,
                    std::map<std::string, std::string> &auth, bool is_proxy) {
                    auto auth_key = is_proxy ? "Proxy-Authenticate" :
                                               "WWW-Authenticate";
                ...
                std::string random_string(size_t length) {
                    auto randchar = []() -> char {
                        ...
                        return charset[static_cast<size_t>(std::rand()) %
                                       max_index];
                    };
            ...
            std::pair<std::string, std::string>
                make_basic_authentication_header(const std::string &username,
                                                 ...
                                                 bool is_proxy) {
                auto field = "Basic " +
                             detail::base64_encode(username + ":" + password);
            ...
            void Response::set_content_provider(
                ...
                set_header("Content-Type", content_type);
                content_length_ = 0;
                content_provider_ = detail::ContentProviderAdapter(
                    std::move(provider));
                content_provider_resource_releaser_ = resource_releaser;
                is_chunked_content_provider_ = false;
            }
            ...
            size_t Result::get_request_header_value_count(
            ...
            namespace detail {
                ...
                bool SocketStream::is_writable() const {
                    return select_write(sock_, write_timeout_sec_,
                ...
                ssize_t SocketStream::read(char *ptr, size_t size) {
                    ...
                    if (read_buff_off_ < read_buff_content_size_) {
                        auto remaining_size = read_buff_content_size_ -
                                              read_buff_off_;
                    ...
                    if (size < read_buff_size_) {
                        auto n = read_socket(sock_, read_buff_.data(),
                                             read_buff_size_,
                                             CPPHTTPLIB_RECV_FLAGS);
                        if (n <= 0) {
                            return n;
                        } else if (n <= static_cast<ssize_t>(size)) {
                            memcpy(ptr, read_buff_.data(),
                        ...
                        return read_socket(sock_, ptr, size,
                                           CPPHTTPLIB_RECV_FLAGS);
                    }
                }
            ...
            Server &Server::Options(const std::string &pattern,
                                    Handler handler) {
            ...
            bool Server::remove_mount_point(const std::string &mount_point) {
                for (auto it = base_dirs_.begin(); it != base_dirs_.end();
                     ++it) {
            ...
            Server &Server::set_file_extension_and_mimetype_mapping(
            ...
            bool Server::listen(const std::string &host, int port,
                                int socket_flags) {
                return bind_to_port(host, port, socket_flags) &&
                       listen_internal();
            }
            ...
            void Server::stop() {
                if (is_running_) {
                    ...
                    std::atomic<socket_t> sock(
                        svr_sock_.exchange(INVALID_SOCKET));
            ...
            bool Server::write_response(Stream &strm, bool close_connection,
                                        const Request &req, Response &res) {
                return write_response_core(strm, close_connection, req, res,
                                           false);
            }
            ...
            bool Server::write_response_core(Stream &strm,
                                             bool close_connection,
                                             const Request &req, Response &res,
            ...
            bool Server::read_content(Stream &strm, Request &req,
                                      Response &res) {
            ...
            bool Server::read_content_core(
                ...
                if (req.is_multipart_form_data()) {
                    const auto &content_type = req.get_header_value(
                        "Content-Type");
                    std::string boundary;
                    if (!detail::parse_multipart_boundary(content_type,
                                                          boundary)) {
                        res.status = 400;
                        return false;
                    }

                    multipart_form_data_parser.set_boundary(
                        std::move(boundary));
                    out = [&](const char *buf, size_t n, uint64_t /*off*/,
                        ...
                        mulitpart_header); if (!ret) { return false; } pos +=
                        read_size;
                        }
                        return true;
                        */
                    ...
                    out = [receiver](const char *buf, size_t n,
                    ...
                    if (!req.path.compare(0, entry.mount_point.size(),
                        ...
                        if (detail::is_valid_path(sub_path)) {
                            ...
                            if (detail::is_file(path)) {
                                ...
                                res.status = req.has_header("Range") ? 206 :
                                                                       200;
            ...
            socket_t Server::create_server_socket(
                ...
                return detail::create_socket(
                    ...
                    tcp_nodelay_, std::move(socket_options),
                    ...
                    while (svr_sock_ != INVALID_SOCKET) {
                        ...
                        if (sock == INVALID_SOCKET) {
                            if (errno == EMFILE) {
                                ...
                                // descriptors has been reached. Try to accept
                                // new connections after a short sleep.
                                std::this_thread::sleep_for(
                                    std::chrono::milliseconds(1));
                                continue;
                            }
            ...
            bool Server::process_request(
                ...
                detail::stream_line_reader line_reader(strm, buf.data(),
                                                       buf.size());
...
#ifdef _WIN32
                // TODO: Increase FD_SETSIZE statically (libzmq), dynamically
                // (MySQL).
#else
                ...
                if (req.has_header("Range")) {
                    const auto &range_header_value = req.get_header_value(
                        "Range");
                ...
                if (req.get_header_value("Expect") == "100-continue") {
                    ...
                    switch (status) {
                        ...
                        case 417:
                            ...
                            break;
                        default:
                            return write_response(strm, close_connection, req,
                                                  res);
                    }
                }
                ...
                if (routed) {
                    ...
                    return write_response_with_content(strm, close_connection,
                                                       req, res);
                } else {
            ...
            bool Server::process_and_close_socket(socket_t sock) {
                auto ret = detail::process_server_socket(
                    svr_sock_, sock, keep_alive_max_count_,
                    keep_alive_timeout_sec_, read_timeout_sec_,
                    read_timeout_usec_, write_timeout_sec_, write_timeout_usec_,
                    [this](Stream &strm, bool close_connection,
            ...
            ClientImpl::ClientImpl(const std::string &host, int port,
                ...
                : host_(host), port_(port),
                  host_and_port_(adjust_host_string(host) + ":" +
                                 std::to_string(port)),
            ...
            void ClientImpl::copy_settings(const ClientImpl &rhs) {
                ...
                proxy_bearer_token_auth_token_ =
                    rhs.proxy_bearer_token_auth_token_;
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
                ...
                server_certificate_verification_ =
                    rhs.server_certificate_verification_;
#endif
                logger_ = rhs.logger_;
            }
            ...
            socket_t ClientImpl::create_client_socket(Error &error) const {
                if (!proxy_host_.empty() && proxy_port_ != -1) {
                    return detail::create_client_socket(
                        ...
                        write_timeout_sec_, write_timeout_usec_, interface_,
                        error);
                }
                ...
                return detail::create_client_socket(
                    ...
                    connection_timeout_usec_, read_timeout_sec_,
                    read_timeout_usec_, write_timeout_sec_, write_timeout_usec_,
                    interface_, error);
            }
            ...
            void ClientImpl::shutdown_ssl(Socket & /*socket*/,
                ...
                // If there are any requests in flight from threads other than
                // us, then it's a thread-unsafe race because individual ssl*
            ...
            void ClientImpl::close_socket(Socket &socket) {
                // If there are requests in flight in another thread, usually
                // closing the socket will be fine and they will simply receive
                // an error when using the closed socket, but it is still a bug
                // since rarely the OS may reassign the socket id to be used for
                // a new socket, and then suddenly they will be operating on a
                // live socket that is different than the one they intended!
                assert(socket_requests_in_flight_ == 0 ||
                       socket_requests_are_from_thread_ ==
                           std::this_thread::get_id());
            ...
            bool ClientImpl::read_response_line(Stream &strm,
                                                const Request &req,
                                                Response &res) {
                std::array<char, 2048> buf{};
            ...
            bool ClientImpl::send(Request &req, Response &res, Error &error) {
...

                {
                    ...
                    // Set this to false immediately - if it ever gets set to
                    // true by the end of the request, we know another thread
                    // instructed us to close the socket.
                    ...
                    if (socket_.is_open()) {
                        ...
                        if (!is_alive) {
                            ...
                            // nongracefully if it seems like the other side has
                            // already closed the connection Also, there cannot
                            // be any requests in flight from other threads
                            // since we locked request_mutex_, so safe to close
                    ...
                    if (!is_alive) {
                        ...
                        if (is_ssl()) {
                            ...
                            if (!proxy_host_.empty() && proxy_port_ != -1) {
                                bool success = false;
                                if (!scli.connect_with_proxy(socket_, res,
                                                             success, error)) {
                                    return success;
                                }
                            }
                    ...
                    // Mark the current socket as being in use so that it cannot
                    // be closed by anyone else while this request is ongoing,
                    // even though we will be releasing the mutex.
            ...
            bool ClientImpl::handle_request(Stream &strm, Request &req,
                ...
                if ((res.status == 401 || res.status == 407) &&
                    ...
                    const auto &username = is_proxy ?
                                               proxy_digest_auth_username_ :
                                               digest_auth_username_;
                    const auto &password = is_proxy ?
            ...
            bool ClientImpl::redirect(Request &req, Response &res,
                                      Error &error) {
                ...
                auto location = detail::decode_url(
                    res.get_header_value("location"), true);
            ...
            bool ClientImpl::write_content_with_provider(Stream &strm,
                ...
                if (req.is_chunked_content_provider_) {
                    ...
                    return detail::write_content_chunked(
                        strm, req.content_provider_, is_shutting_down,
                        *compressor, error);
                } else {
    ...
    bool net::http::ClientImpl::write_request(Stream &strm, Request &req,
        ...
        // Request line and headers
        {
            ...
            const auto &path = url_encode_ ? detail::encode_url(req.path) :
                                             req.path;
    ...
    std::unique_ptr<net::http::Response>
        net::http::ClientImpl::send_with_content_provider(
    ...
    bool ClientImpl::process_request(Stream &strm, Request &req, Response &res,
        ...
        if (res.get_header_value("Connection") == "close" ||
            ...
            // TODO this requires a not-entirely-obvious chain of calls
            // to be correct for this to be safe. Maybe a code refactor
            // (such as moving this out to the send function and getting
            // rid of the recursiveness of the mutex) could make this
            // more obvious.

            // This is safe to call because process_request is only
            // called by handle_request which is only called by send,
            // which locks the request mutex during the process. It
            // would be a bug to call it from a different thread since
            // it's a thread-safety issue to do these things to the
            // socket if another thread is using the socket.
            std::lock_guard<std::mutex> guard(socket_mutex_);
            shutdown_ssl(socket_, true);
            shutdown_socket(socket_);
            close_socket(socket_);
    ...
    Result ClientImpl::Get(const std::string &path, const Params &params,
                           ...
                           Progress progress) {
        if (params.empty()) {
            return Get(path, headers, response_handler, content_receiver,
                       progress);
        }
    ...
    Result ClientImpl::Put(const std::string &path, const Headers &headers,
                           ContentProviderWithoutLength content_provider,
                           const std::string &content_type) {
        return send_with_content_provider("PUT", path, headers, nullptr, 0,
                                          nullptr, std::move(content_provider),
                                          content_type);
    }
    ...
    Result ClientImpl::Patch(const std::string &path,
                             ContentProviderWithoutLength content_provider,
                             const std::string &content_type) {
        return Patch(path, Headers(), std::move(content_provider),
                     content_type);
    }
    ...
    void ClientImpl::stop() {
        ...
        // If there is anything ongoing right now, the ONLY thread-safe
        // thing we can do is to shutdown_socket, so that threads using
        // this socket suddenly discover they can't read/write any more
        // and error out. Everything else (closing the socket, shutting
        // ssl down) is unsafe because these actions are not
        // thread-safe.
        if (socket_requests_in_flight_ > 0) {
            shutdown_socket(socket_);

            // Aside from that, we set a flag for the socket to be
            // closed when we're done.
            socket_should_be_closed_when_request_is_done_ = true;
            return;
        }

        // Otherwise, sitll holding the mutex, we can shut everything
        // down ourselves
        shutdown_ssl(socket_, true);
        shutdown_socket(socket_);
        close_socket(socket_);
 ...
 */
    ...
    namespace detail {
        ...
        void ssl_delete(std::mutex &ctx_mutex, SSL *ssl,
                        bool shutdown_gracefully) {
            // sometimes we may want to skip this to try to avoid
            // SIGPIPE if we know the remote has closed the network
            // connection Note that it is not always possible to avoid
            // SIGPIPE, this is merely a best-efforts.
        ...
        bool SSLSocketStream::is_readable() const {
            return detail::select_read(sock_, read_timeout_sec_,
                                       read_timeout_usec_) > 0;
        }
    ...
    SSLServer::SSLServer(const char *cert_path, const char *private_key_path,
        ...
        if (ctx_) {
            ...
            // add default password callback before opening encrypted
            // private key
    ...
    SSLClient::~SSLClient() {
        if (ctx_) { SSL_CTX_free(ctx_); }
        // Make sure to shut down SSL since shutdown_ssl will resolve to
        // the base function rather than the derived function once we
        // get to the base class destructor, and won't free the SSL
        // (causing a leak).
        ...
        if (ca_cert_store) {
            if (ctx_) {
                if (SSL_CTX_get_cert_store(ctx_) != ca_cert_store) {
                    // Free memory allocated for old cert and use new
                    // store `ca_cert_store`
    ...
    // Assumes that socket_mutex_ is locked and that there are no
    // requests in flight
            ...
            if (!proxy_digest_auth_username_.empty() &&
                ...
                if (detail::parse_www_authenticate(res2, auth, true)) {
                    ...
                    if (!detail::process_client_socket(
                        ...
                        // Thread-safe to close everything because we
                        // are assuming there are no requests in flight
                        shutdown_ssl(socket, true);
                        shutdown_socket(socket);
                        close_socket(socket);
                        success = false;
    ...
    bool SSLClient::verify_host(X509 *server_cert) const {
        /* Quote from RFC2818 section 3.1 "Server Identity"
           ...
           certificate MUST be used. Although the use of the Common Name
           is existing practice, it is deprecated and Certification
           Authorities are encouraged to use the dNSName instead.

           Matching is performed using the matching rules specified by
           [RFC2459].  If more than one identity of a given type is
           present in the certificate (e.g., more than one dNSName name,
           ...
           than a hostname. In this case, the iPAddress subjectAltName
           must be present in the certificate and must exactly match the
           IP in the URI.
    ...
    bool SSLClient::verify_host_with_subject_alt_name(X509 *server_cert) const {
        ...
        auto alt_names = static_cast<const struct stack_st_GENERAL_NAME *>(
            X509_get_ext_d2i(server_cert, NID_subject_alt_name, nullptr,
                             nullptr));
    ...
    Result Client::Head(const std::string &path) {
